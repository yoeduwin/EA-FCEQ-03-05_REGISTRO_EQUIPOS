 ...
 *
 * COPIA EN REPO `formatos` — idéntica a la de SEA salvo la opción authAction,
 * retrocompatible: sin pasarla, el comportamiento es el mismo.
 */
...
const SEAAuth = (() => {
  ...
  let _authAction = 'verificarAcceso'; // acción que valida la whitelist en el GAS
   ...
   *   authAction: acción del GAS que valida la whitelist. Default 'verificarAcceso'
   *   (sin restricción de módulo). Cambiar solo si se agrega una acción propia.
   */
  function init(onAuthSuccess, options) {
    _onAuthSuccess = onAuthSuccess;
    _gasUrl = (options && options.gasUrl) || null;
    _authAction = (options && options.authAction) || 'verificarAcceso';
    ...
    if (_gasUrl) {
      try {
        ...
        const resp = await fetch(_gasUrl, {
          ...
          body: JSON.stringify({ action: _authAction, id_token: token })
        });
